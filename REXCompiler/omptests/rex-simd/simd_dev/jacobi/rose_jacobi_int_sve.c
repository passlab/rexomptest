...
void initialize(int n,int m,int alpha,int *dx,int *dy,int *u_p,int *f_p)
{
  int i;
  int j;
  int xx;
...
void error_check(int n,int m,int alpha,int dx,int dy,int *u_p,int *f_p)
{
  int i;
  int j;
  int xx;
...
void jacobi_seq(int n,int m,int dx,int dy,int alpha,int relax,int *u_p,int *f_p,int tol,int mits);
void jacobi_omp(int n,int m,int dx,int dy,int alpha,int relax,int *u_p,int *f_p,int tol,int mits);
...
/* the rest of arg ignored */
  ...
  printf("------------------------------------------------------------------------------------------------------\n");
/** init the array */
  int *u = (int *)(malloc(sizeof(int ) * n * m));
  int *uomp = (int *)(malloc(sizeof(int ) * n * m));
...
void jacobi_seq(int n,int m,int dx,int dy,int alpha,int omega,int *u_p,int *f_p,int tol,int mits)
{
  int i;
  int j;
  int k;
...
void jacobi_omp(int n,int m,int dx,int dy,int alpha,int omega,int *u_p,int *f_p,int tol,int mits)
{
  int i;
  int j;
  int k;
...
/* Copy new solution into old */
    for (i = 0; i < n; i++) {
      ...
      for (j = 0; j <= m - 1; j += svcntw()) {
        int *__ptr0 = uold[i];
        int *__ptr1 = u[i];
        svint32_t __vec2 = svld1(__pg0,&__ptr1[j]);
