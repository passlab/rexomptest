...
/* Create the source (x) vector */
  ...
  for (row = 0; row < nrows; row++) {
    ...
    for (idx = ia[row]; idx <= (ia[row + 1] - 1); idx += 8 * svcntw()) {
      ...
      __part0 = (__vec4);
      svfloat32_t __vec5 = svld1(__pg0,&a[idx + svcntw() * 1]);
      svint32_t __vindex1 = svld1(__pg0,&ja[idx + 1 * 1]);
      ...
      __part0 = (__vec8);
      svfloat32_t __vec9 = svld1(__pg0,&a[idx + svcntw() * 2]);
      svint32_t __vindex2 = svld1(__pg0,&ja[idx + 1 * 2]);
      ...
      __part0 = (__vec12);
      svfloat32_t __vec13 = svld1(__pg0,&a[idx + svcntw() * 3]);
      svint32_t __vindex3 = svld1(__pg0,&ja[idx + 1 * 3]);
      ...
      __part0 = (__vec16);
      svfloat32_t __vec17 = svld1(__pg0,&a[idx + svcntw() * 4]);
      svint32_t __vindex4 = svld1(__pg0,&ja[idx + 1 * 4]);
      svfloat32_t __vec18 = svld1_gather_index(__pg0,x,__vindex4);
      svfloat32_t __vec19 = svmul_f32_m(__pg0,__vec18,__vec17);
      svfloat32_t __vec20 = svadd_f32_m(__pg0,__vec19,__part0);
      __part0 = (__vec20);
      svfloat32_t __vec21 = svld1(__pg0,&a[idx + svcntw() * 5]);
      svint32_t __vindex5 = svld1(__pg0,&ja[idx + 1 * 5]);
      svfloat32_t __vec22 = svld1_gather_index(__pg0,x,__vindex5);
      svfloat32_t __vec23 = svmul_f32_m(__pg0,__vec22,__vec21);
      svfloat32_t __vec24 = svadd_f32_m(__pg0,__vec23,__part0);
      __part0 = (__vec24);
      svfloat32_t __vec25 = svld1(__pg0,&a[idx + svcntw() * 6]);
      svint32_t __vindex6 = svld1(__pg0,&ja[idx + 1 * 6]);
      svfloat32_t __vec26 = svld1_gather_index(__pg0,x,__vindex6);
      svfloat32_t __vec27 = svmul_f32_m(__pg0,__vec26,__vec25);
      svfloat32_t __vec28 = svadd_f32_m(__pg0,__vec27,__part0);
      __part0 = (__vec28);
      svfloat32_t __vec29 = svld1(__pg0,&a[idx + svcntw() * 7]);
      svint32_t __vindex7 = svld1(__pg0,&ja[idx + 1 * 7]);
      svfloat32_t __vec30 = svld1_gather_index(__pg0,x,__vindex7);
