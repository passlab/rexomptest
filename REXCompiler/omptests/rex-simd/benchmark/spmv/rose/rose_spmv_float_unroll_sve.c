...
/* Create the source (x) vector */
  ...
  for (row = 0; row < nrows; row++) {
    ...
    for (idx = ia[row]; idx <= (ia[row + 1] - 1); idx += 4 * svcntw()) {
      ...
      svfloat32_t __vec7 = svld1_gather_index(__pg0,x,__vindex1);
      ...
      svfloat32_t __vec12 = svld1_gather_index(__pg0,x,__vindex2);
      ...
      svfloat32_t __vec17 = svld1_gather_index(__pg0,x,__vindex3);
