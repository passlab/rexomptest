...
int main (int argc, char **argv) {
    ...
    layer2->out_h   = (layer2->w+2*layer2->padding-layer2->size)/layer2->stride+1;
    layer2->out_w   = (layer2->h+2*layer2->padding-layer2->size)/layer2->stride+1;
    ...
    */
    ...
    // timer
    double tmp, tmp_forward, tmp_backward, tmp_update, tmp_total;
    double time_io_read, time_io_write;
    double time_forward, time_backward, time_update, time_total;
    ...
    */
    ...
    int HWC_conv1_tensor  = batch*height_col0*width_col0*channels_col0;                                                     // malloc
    int HWC_conv1_t1      = height_col0*width_col0*channels_col0; // = N0*M0
    int HWC_conv1_t2      = M0*K0*N0;

    float *conv1_tensor = (float *)malloc(HWC_conv1_tensor*sizeof(float));
    float *conv1_t1     = (float *)malloc(HWC_conv1_t1*sizeof(float));
    float *conv1_t2     = (float *)malloc(HWC_conv1_t2*sizeof(float));
    int HWC_conv1_weights = network->layers[0]->n*network->layers[0]->size*network->layers[0]->size*network->layers[0]->c; // tofrom
    ...
    int HWC_conv2_tensor  = batch*height_col2*width_col2*channels_col2;                                                    // malloc
    int HWC_conv2_t1      = height_col2*width_col2*channels_col2;
    int HWC_conv2_t2      = M2*K2*N2;

    float *conv2_tensor = (float *)malloc(HWC_conv2_tensor*sizeof(float));
    float *conv2_t1     = (float *)malloc(HWC_conv2_t1*sizeof(float));
    float *conv2_t2     = (float *)malloc(HWC_conv2_t2*sizeof(float));
    int HWC_conv2_weights = network->layers[2]->n*network->layers[2]->size*network->layers[2]->size*network->layers[2]->c; // tofrom
    ...
    for (int i_epoch = 0; i_epoch < training_epoch; i_epoch++) {
	    ...
	    // conv
	    conv(network->layers[0]->batch, M0, K0, N0, channels_col0, height_col0, width_col0, ksize0, stride0, channels0, height0, width0, pad0, network->input, network->layers[0]->output, network->layers[0]->weights, conv1_tensor);
	    ...
	    // conv
	    conv(network->layers[2]->batch, M2, K2, N2, channels_col2, height_col2, width_col2, ksize2, stride2, channels2, height2, width2, pad2, network->layers[1]->output, network->layers[2]->output, network->layers[2]->weights, conv2_tensor);
	    ...
	    tmp = read_timer_ms();
            ...
            // gradient array
            n = network->layers[5]->batch*N5;
            for(int i = 0; i < n; i++) {if (network->layers[5]->output[i] <= 0) network->layers[5]->delta[i] = 0;}
            // backward array
	    ...
	    // gradient array
            n = network->layers[4]->batch*N4;
            for(int i = 0; i < n; i++) {if (network->layers[4]->output[i] <= 0) network->layers[4]->delta[i] = 0;}
            // backward array
	    ...
	    // maxpool backward
	    max_pool_backward(network->layers[3]->batch, N3, M3, network->layers[3]->indexes, network->layers[3]->delta, network->layers[2]->delta);
	    ...
	    // gradient array
            n = network->layers[2]->batch*N2*M2;
            for(int i = 0; i < n; i++) {if (network->layers[2]->output[i] <= 0) network->layers[2]->delta[i] = 0;}
            
	    // backward bias
            bias_backward(network->layers[2]->batch, N2, M2, network->layers[2]->delta, network->layers[2]->bias_updates);

	    // conv backward
	    conv_backward(network->layers[2]->batch, K2, N2, M2, channels_col2, height_col2, width_col2, ksize2, stride2, channels2, height2, width2, pad2, network->layers[1]->output, network->layers[2]->delta, network->layers[2]->weight_updates, network->layers[1]->delta, network->layers[2]->weights, conv2_t1, conv2_t2);
	    ...
	    tmp = read_timer_ms();
            ...
            // maxpool backward
            max_pool_backward(network->layers[1]->batch, N1, M1, network->layers[1]->indexes, network->layers[1]->delta, network->layers[0]->delta);
            ...
            // gradient array
            n = network->layers[0]->batch*N0*M0;
            for(int i = 0; i < n; i++) {if (network->layers[0]->output[i] <= 0) network->layers[0]->delta[i] = 0;}
            
	    // backward bias
            bias_backward(network->layers[0]->batch, N0, M0, network->layers[0]->delta, network->layers[0]->bias_updates);
           
	    // conv backward
            conv_backward(network->layers[0]->batch, K0, N0, M0, channels_col0, height_col0, width_col0, ksize0, stride0, channels0, height0, width0, pad0, network->layers[0]->output, network->layers[0]->delta, network->layers[0]->weight_updates, network->layers[0]->delta, network->layers[0]->weights, conv1_t1, conv1_t2);
	    ...
	    time_update = read_timer_ms() - tmp_update;
            ...
            time_total = read_timer_ms() - tmp_total;
            printf("total epoch# %d batch# %d device# %d: %lf\n", i_epoch, i_batch, dev_id, time_total);
