...
//printf("num_omp_threads: %d\n", num_omp_threads);
  ...
  for (r = 0; r < row; r++) {
    __m256 __vec3 = _mm256_set1_ps(delta);
    __m256 __part0 = _mm256_setzero_ps();
    __m256 __vec1 = _mm256_set1_ps(0);
    for (c = 0; c <= col - 1; c += 1 * 8) {
      __part0 = (__vec1);
      __m256 __vec2 = _mm256_loadu_ps(&temp[r * col + c]);
      __m256 __vec4 = _mm256_add_ps(__vec3,__vec2);
      _mm256_storeu_ps(&result[r * col + c],__vec4);
    }
