...
//printf("num_omp_threads: %d\n", num_omp_threads);
  ...
  for (r = 0; r < row; r++) {
    __m512 __vec3 = _mm512_set1_ps(delta);
    __m512 __part0 = _mm512_setzero_ps();
    __m512 __vec1 = _mm512_set1_ps(0);
    for (c = 0; c <= col - 1; c += 1 * 16) {
      __part0 = (__vec1);
      __m512 __vec2 = _mm512_loadu_ps(&temp[r * col + c]);
      __m512 __vec4 = _mm512_add_ps(__vec3,__vec2);
      _mm512_storeu_ps(&result[r * col + c],__vec4);
    }
