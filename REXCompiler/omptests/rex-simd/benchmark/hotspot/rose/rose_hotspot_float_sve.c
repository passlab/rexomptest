...
void single_iteration(float *result,float *temp,float *power,int row,int col,float Cap,float Rx,float Ry,float Rz,float step)
{
  float delta;
  int r;
  int c;
//printf("num_omp_threads: %d\n", num_omp_threads);
  ...
  for (r = 0; r < row; r++) {
    svfloat32_t __vec3 = svdup_f32(delta);
    svfloat32_t __part0 = svdup_f32(0.00000L);
    svfloat32_t __vec1 = svdup_f32(0);
    svbool_t __pg0 = svwhilelt_b32((unsigned long )0,(unsigned long )(col - 1));
    for (c = 0; c <= (col - 1); c += 1 * svcntw()) {
      __part0 = (__vec1);
      svfloat32_t __vec2 = svld1(__pg0,&temp[r * col + c]);
      svfloat32_t __vec4 = svadd_f32_m(__pg0,__vec3,__vec2);
      svst1(__pg0,&result[r * col + c],__vec4);
      __pg0 = svwhilelt_b32((unsigned long )c,(unsigned long )(col - 1));
    }
    __pg0 = svptrue_b32();
    delta += svaddv(__pg0,__part0);
  }
  for (r = 0; r < row; r++) {
    for (c = 0; c < col; c++) {
/*	Corner 1	*/
...
void compute_tran_temp(float *result,int num_iterations,float *temp,float *power,int row,int col)
{
  ...
  float Cap = (0.5 * 1.75e6 * t_chip * grid_width * grid_height);
  float Rx = (grid_width / (2.0 * 100 * t_chip * grid_height));
  float Ry = (grid_height / (2.0 * 100 * t_chip * grid_width));
  float Rz = t_chip / (100 * grid_height * grid_width);
  float max_slope = (3.0e6 / (0.5 * t_chip * 1.75e6));
  float step = (0.001 / max_slope);
  float t;
	#ifdef VERBOSE
	#endif
