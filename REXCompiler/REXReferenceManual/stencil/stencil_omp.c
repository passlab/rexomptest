...
void stencil_omp_target(REAL* src, REAL* dst, int width, int height, float* filter, int flt_width, int flt_height) {
    ...
    int N = width*height;
    int i, j;
#pragma omp target map(to: width, height, flt_width, flt_height, src[0:N], filter[0:flt_size]) map(from: dst[0:N])
//#pragma omp parallel for num_threads(TEAM_SIZE) collapse(2)
